...
export default {
  ...
  methods: {
    ...
    getMeteoUrl(lat, lng) {
      const options =
        "rain,wind_speed_10m,relative_humidity_2m,temperature_2m,is_day,cloud_cover";
    ...
    getCoordenates(callback = () => {}) {
      if ("geolocation" in navigator) {
        navigator.geolocation.getCurrentPosition(
          ...
          (error) => {
            console.debug(
              `Error getCoordenates(${error.code}): ${error.message}`
            );
